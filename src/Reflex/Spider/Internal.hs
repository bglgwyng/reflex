...
#ifdef DONT_USE_APPLICATIVE_DYNAMIC
                print ("updateMe4", oldParentHeight)
#else
                print ("updateMe3", oldParentHeight)
#endif
...
#if DONT_USE_APPLICATIVE_DYNAMIC
  df <*> dx = df >>= \f -> dx >>= \x -> pure (f x)
#else
#if MIN_VERSION_base(4,10,0)
